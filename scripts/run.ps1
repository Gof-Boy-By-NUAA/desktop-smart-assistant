...
<#
.SYNOPSIS
    SmartAssistant installer & management script for Windows.
.DESCRIPTION
    One-liner install:
...
function Install-Project {
    if (Test-Path "SmartAssistant") {
        # Auto-backup the existing directory without prompting.
        $backup = "SmartAssistant_backup_$(Get-Date -Format 'yyyyMMddHHmmss')"
        Rename-Item "SmartAssistant" $backup
        Write-Warn ((T "已存在 SmartAssistant 目录，已自动备份为" "Existing 'SmartAssistant' directory backed up to") + " '$backup'")
    }
    ...
    Write-Cow (T "正在克隆 SmartAssistant 项目..." "Cloning SmartAssistant project...")
    $cloneOk = $false
    ...
    try {
        ...
        git clone --depth 10 --progress "https://github.com/zhayujie/SmartAssistant.git" 2>&1 | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -eq 0) { $cloneOk = $true }
        $ErrorActionPreference = $prevEAP
        if (-not $cloneOk) {
            if (Test-Path "SmartAssistant") { Remove-Item -Recurse -Force "SmartAssistant" }
        }
    } catch {}
    ...
    if (-not $cloneOk) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        git clone --depth 10 --progress "https://gitee.com/zhayujie/SmartAssistant.git" 2>&1 | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -eq 0) { $cloneOk = $true }
        $ErrorActionPreference = $prevEAP
        if (-not $cloneOk) {
            if (Test-Path "SmartAssistant") { Remove-Item -Recurse -Force "SmartAssistant" }
    ...
    if (-not $cloneOk) {
        ...
        Write-Err (T "你也可以手动克隆: git clone https://gitee.com/zhayujie/SmartAssistant.git" "You can also manually clone: git clone https://gitee.com/zhayujie/SmartAssistant.git")
        Read-Host (T "按回车退出" "Press Enter to exit")
        exit 1
    }

    Set-Location "SmartAssistant"
...
# ── start via cow CLI ─────────────────────────────────────────────
function Start-SmartAssistant {
    Write-Cow (T "正在启动 SmartAssistant..." "Starting SmartAssistant...")
...
function Show-Usage {
    Write-Info "========================================="
    Write-Info "   SmartAssistant Management Script (Windows)"
    Write-Info "========================================="
    Write-Host ""
...
function Install-Mode {
    Clear-Host
    Write-Info "========================================="
    Write-Info "   SmartAssistant Installation (Windows)"
    Write-Info "========================================="
    Write-Host ""
    ...
    Start-SmartAssistant
}

# ── update ────────────────────────────────────────────────────────
function Update-Project {
    Write-Cow (T "正在更新 SmartAssistant..." "Updating SmartAssistant...")
    ...
    if (Test-Path "$BaseDir\.git") {
        ...
        if ($pullExit -ne 0) {
            ...
            git remote set-url origin https://gitee.com/zhayujie/SmartAssistant.git 2>&1 | Out-Null
            git pull 2>&1 | Out-Null
            $ErrorActionPreference = $prevEAP
        }
    } else {
