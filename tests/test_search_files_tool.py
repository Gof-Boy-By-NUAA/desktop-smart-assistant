...
def test_symlink_to_credential_file_is_skipped_not_opened(tmp_path, monkeypatch):
    ...
    workspace.mkdir()
    _symlink_or_skip(workspace / "decoy.txt", fake_cow / "secret.env")
...
@pytest.mark.parametrize("output_mode", ["content", "files", "count"])
def test_every_backend_prunes_knowledge_from_broad_search(
    backend_tool, tmp_path, output_mode
):
    _write(tmp_path, "knowledge/private.md", "BOUNDARY_CANARY\n")
    _write(tmp_path, "visible.txt", "BOUNDARY_CANARY\n")

    result = backend_tool.execute(
        {
            "pattern": "BOUNDARY_CANARY",
    ...
    assert result.status == "success"
    assert "knowledge/private.md" not in str(result.result).replace("\\", "/")
    assert "visible.txt" in str(result.result)


def test_file_name_search_prunes_knowledge_from_workspace_root(tmp_path):
    _write(tmp_path, "knowledge/private.md", "secret\n")
    _write(tmp_path, "visible/private.md", "visible\n")
    ...
    assert result.status == "success"
    assert result.result["files"] == ["visible/private.md"]
