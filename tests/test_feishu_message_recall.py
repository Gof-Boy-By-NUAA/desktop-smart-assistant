...
try:
    ...
    _HAS_FEISHU_TEST_DEPS = True
except (ImportError, ValueError):
    _HAS_FEISHU_TEST_DEPS = False


feishu_deps_required = pytest.mark.skipif(
    not _HAS_FEISHU_TEST_DEPS,
    reason="飞书通道测试需要 requirements.txt 中的 web.py",
)
...
@feishu_deps_required
def test_feishu_message_uses_message_id_for_precise_recall(monkeypatch):
    monkeypatch.setattr(feishu_channel, "_ensure_lark_imported", lambda: None)
...
@feishu_deps_required
def test_feishu_recall_ignores_unknown_message(monkeypatch):
    monkeypatch.setattr(feishu_channel, "_ensure_lark_imported", lambda: None)
    channel = FeiShuChanel()
    channel._message_sessions = ExpiredDict(60)
