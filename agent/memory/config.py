...
class MemoryConfig:
    ...
    max_results: int = 10
    min_score: float = 0.1
    enable_governed_retrieval: bool = True
    ...
    def get_retrieval_db_path(self) -> Path:
        """获取租户化词法检索索引路径。"""

        index_dir = self.get_memory_dir() / "long-term"
        index_dir.mkdir(parents=True, exist_ok=True)
        return index_dir / "retrieval-v2.db"

    def get_governance_db_path(self) -> Path:
        """获取受治理记忆事实库路径。"""

        index_dir = self.get_memory_dir() / "long-term"
        index_dir.mkdir(parents=True, exist_ok=True)
        return index_dir / "governance.db"
