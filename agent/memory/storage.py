...
class MemoryStorage:
    ...
    def delete_by_source(self, source: str) -> int:
        ...
        with self._lock:
            ...
            try:
                ...
                self.conn.execute(
                    "DELETE FROM chunks WHERE source = ?", (normalized_source,)
                )
                self.conn.execute(
                    "DELETE FROM files WHERE source = ?", (normalized_source,)
                )
            except Exception:
                self.conn.rollback()
                raise
    ...
    def delete_missing_file_sources(self, current_paths: List[str]) -> int:
        """删除工作区中已经消失的记忆和知识文件索引。"""

        current = set(current_paths)
        with self._lock:
            rows = self.conn.execute(
                "SELECT path FROM files WHERE source IN ('memory', 'knowledge')"
            ).fetchall()
            stale_paths = [row["path"] for row in rows if row["path"] not in current]
            if not stale_paths:
                return 0
            self.conn.execute("BEGIN IMMEDIATE")
            try:
