...
# Web sessions are security principals, not merely opaque client-selected IDs.
# Existing web rows are deliberately assigned to the legacy/local principal so
# a newly authenticated remote principal cannot claim historical sessions by
# guessing their IDs after an upgrade.
_MIGRATION_ADD_OWNER_ID = """
ALTER TABLE sessions ADD COLUMN owner_id TEXT NOT NULL DEFAULT '';
"""

_MIGRATION_ADD_STATE = """
ALTER TABLE sessions ADD COLUMN state TEXT NOT NULL DEFAULT 'active';
"""

_LEGACY_WEB_OWNER_ID = "web:legacy"
...
def _extract_tool_results(content: Any) -> Dict[str, dict]:
    ...
    for b in content:
        ...
        # 智能体循环把所有工具结果存为严格 JSON。历史界面应把 JSON 字符串
        # 还原成原文本，避免普通命令输出在重新加载后多出一层引号。
        if isinstance(result_content, str):
            try:
                decoded = json.loads(result_content)
            except (json.JSONDecodeError, ValueError, TypeError):
                decoded = result_content
...
class ConversationStore:
    ...
    def healthcheck(self) -> bool:
        """Return whether the SQLite store and required schema are readable."""
        with self._lock:
            conn = self._connect()
            try:
                ...
                return {"session_id", "owner_id", "state"} <= columns
            finally:
                conn.close()
    ...
    def claim_session(
        ...
        """Atomically create or verify a session ownership claim.

        Client-selected ``session_id`` values are locators only.  The durable
        ``owner_id`` is the authorization boundary.  A conflicting claim is
        rejected inside the same transaction, before any message, cancellation,
        steering, or history side effect can occur.
        """
        session_id = self._validated_session_id(session_id)
        owner_id = self._validated_owner_id(owner_id)
        now = int(time.time())
        with self._lock:
            conn = self._connect()
            try:
                with conn:
                    conn.execute(
                        ...
                        INSERT OR IGNORE INTO sessions
                            (session_id, channel_type, owner_id, created_at, last_active, msg_count)
                        VALUES (?, ?, ?, ?, ?, 0)
                        """,
                        (session_id, channel_type, owner_id, now, now),
                    )
                    self._assert_session_owner(conn, session_id, owner_id)
                return True
            finally:
                conn.close()

    def active_session_owner(self, session_id: str) -> Optional[str]:
        """Return active owner (``""`` for legacy-unowned), or ``None``."""
        try:
            session_id = self._validated_session_id(session_id)
    ...
    def owns_session(self, session_id: str, owner_id: str) -> bool:
        """Return whether ``owner_id`` owns an existing session."""
        try:
            session_id = self._validated_session_id(session_id)
            owner_id = self._validated_owner_id(owner_id)
        except (TypeError, ValueError):
        ...
        with self._lock:
            ...
            try:
                row = conn.execute(
                    "SELECT owner_id, state FROM sessions WHERE session_id = ?",
                    (session_id,),
                ).fetchone()
                return bool(
                    row and str(row[0]) == owner_id and str(row[1]) == "active"
                )
            finally:
                conn.close()
    ...
    def append_messages(
        ...
        Args:
            ...
            owner_id: Durable authorization principal. ``None`` is reserved for
                      trusted non-Web callers; Web paths must always provide it.
        """
        if not messages:
            return
        ...
        session_id = self._validated_session_id(session_id)
        owner_value = (
            self._validated_owner_id(owner_id) if owner_id is not None else None
        )
        now = int(time.time())
        with self._lock:
            conn = self._connect()
            try:
                with conn:
                    ...
                    conn.execute(
                        ...
                        VALUES (?, ?, ?, ?, ?, 0)
                        """,
                        (session_id, channel_type, owner_value or "", now, now),
                    )
                    self._assert_session_owner(conn, session_id, owner_value)
                    conn.execute(
    ...
    def delete_session(self, session_id: str, owner_id: str) -> None:
        """Idempotently tombstone a Web session without cross-owner leakage."""
        session_id = self._validated_session_id(session_id)
        owner_id = self._validated_owner_id(owner_id)
        now = int(time.time())
        with self._lock:
            ...
            try:
                with conn:
                    ...
                    if not row or str(row[0]) != owner_id:
                        raise PermissionError("session not found")
                    if str(row[1]) == "deleted":
                        return
                    if str(row[1]) != "active":
                        raise PermissionError("session not found")
                    conn.execute(
                    ...
                    conn.execute(
                        """
                        UPDATE sessions
                        SET state = 'deleted', title = '', context_start_seq = 0,
                            last_active = ?, msg_count = 0
                        WHERE session_id = ?
                        """,
                        (now, session_id),
                    )
    ...
    def list_sessions(
        ...
        """List sessions visible to the optional durable owner principal."""
        page = max(1, page)
        page_size = max(1, min(int(page_size), 200))
        conditions: List[str] = ["state = 'active'"]
        params: List[Any] = []
        if channel_type:
        ...
        if owner_id is not None:
            ...
            params.append(self._validated_owner_id(owner_id))
        ...
        with self._lock:
            ...
            try:
                total = conn.execute(
                    "SELECT COUNT(*) FROM sessions" + where, tuple(params)
                ).fetchone()[0]
                rows = conn.execute(
                    """
    ...
    def rename_session(
        self, session_id: str, title: str, owner_id: Optional[str] = None
    ) -> bool:
    ...
    def _assert_session_owner(
        ...
        """Fail closed without disclosing whether a foreign session exists."""
        session_id = cls._validated_session_id(session_id)
        validated_owner = (
            cls._validated_owner_id(owner_id) if owner_id is not None else None
        )
        row = conn.execute(
            "SELECT owner_id, state FROM sessions WHERE session_id = ?",
            (session_id,),
        ...
        if validated_owner is None:
            if stored_owner:
                raise PermissionError("session not found")
            return
        if stored_owner != validated_owner:
            raise PermissionError("session not found")
            ...
            try:
                ...
                if conn is not None:
                    try:
                        ...
                        pass
                # Multiple processes can start against the same legacy DB. Let
                # the winner commit, then re-open and re-check the actual schema.
    ...
    def _add_column_strict(
        ...
        try:
            ...
            # A concurrent migrator may have committed the same ALTER between
    ...
    def _migrate(self, conn: sqlite3.Connection) -> None:
        """Apply structural migrations; never continue with a partial schema."""
        migrations = (
            ("sessions", "channel_type", _MIGRATION_ADD_CHANNEL_TYPE),
            ("sessions", "title", _MIGRATION_ADD_TITLE),
            ("sessions", "context_start_seq", _MIGRATION_ADD_CONTEXT_START_SEQ),
            ("sessions", "owner_id", _MIGRATION_ADD_OWNER_ID),
            ("sessions", "state", _MIGRATION_ADD_STATE),
            ("messages", "extras", _MIGRATION_ADD_MSG_EXTRAS),
        )
        for table, column, ddl in migrations:
            if self._add_column_strict(conn, table, column, ddl):
                logger.info(
                    f"[ConversationStore] Migrated: added {table}.{column}"
                )

        # Existing Web sessions become a non-claimable local/legacy principal.
        # This update and its lookup index are part of the security migration.
        conn.execute(
            "UPDATE sessions SET owner_id = ? "
            "WHERE owner_id = '' AND channel_type = 'web'",
            (_LEGACY_WEB_OWNER_ID,),
        )
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_sessions_owner_channel_state_active "
            "ON sessions (owner_id, channel_type, state, last_active DESC)"
        )
        conn.commit()
