...
class ToolManager:
    ...
    def _mcp_json_path(self) -> str:
        import os
        workspace = os.path.expanduser(conf().get("agent_workspace", "./workspace"))
        return os.path.join(workspace, "mcp.json")

    def _read_mcp_json_signature(self):
        """
        Return (mtime, sha256_of_bytes) for the workspace mcp.json without parsing.
    ...
    def _load_mcp_configs(self) -> list:
        ...
        Load MCP server configs with priority:
          1. workspace/mcp.json  (supports both mcpServers and mcp_servers keys)
    ...
    def refresh_mcp_if_changed(self):
        """
        Cheap check whether workspace/mcp.json has changed since last load.
    ...
    def sync_mcp_into_agent(self, agent) -> tuple:
        ...
        Handles both representations SmartAssistant uses:
    ...
    def create_tool(
        ...
        """按名称创建工具，并在构造阶段合并本次运行所需的配置。"""
        tool_class = self.tool_classes.get(name)
        if tool_class:
            configured = hasattr(self, "tool_configs") and name in self.tool_configs
            configured = configured or runtime_config is not None
            if not configured:
                return tool_class()

            merged_config = dict(
                (getattr(self, "tool_configs", {}) or {}).get(name) or {}
            )
            if runtime_config is not None:
                if not isinstance(runtime_config, dict):
                    raise TypeError("runtime_config must be a dict")
                merged_config.update(runtime_config)

            # 优先把配置交给构造器，确保 cwd、超时和安全开关不会在初始化后失配。
            keyword_config = False
            positional_config = False
            try:
                ...
                if config_parameter is not None:
                    if config_parameter.kind is inspect.Parameter.POSITIONAL_ONLY:
                        signature.bind(merged_config)
                        positional_config = True
                    else:
                        signature.bind(config=merged_config)
                        keyword_config = True
                elif any(
            ...
            if keyword_config:
                tool_instance = tool_class(config=merged_config)
            elif positional_config:
                tool_instance = tool_class(merged_config)
            else:
                # 无参工具保持原有构造协议，但仍可读取合并后的通用配置。
                tool_instance = tool_class()
                tool_instance.config = merged_config
