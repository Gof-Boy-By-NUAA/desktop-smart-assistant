...
class Send(BaseTool):
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        # 通用发送工具不得绕过知识和记忆的专用访问边界。
        if is_machine_managed_knowledge_path(absolute_path, self.cwd):
            return ToolResult.fail(
                "Error: governed knowledge cannot be sent with the send tool; "
                "use knowledge_search or knowledge_get"
            )
        if is_governed_private_path(absolute_path, self.cwd):
            return ToolResult.fail(
                "Error: governed private data cannot be sent with the send tool"
            )
