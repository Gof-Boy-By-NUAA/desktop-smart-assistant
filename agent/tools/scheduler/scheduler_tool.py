...
class SchedulerTool(BaseTool):
    ...
    def execute(self, params: dict) -> ToolResult:
        ...
        try:
            ...
            # The tool methods predate ToolResult and express validation
            # failures as a localized error string. Never turn that into a
            # successful tool call: callers use the status to decide whether
            # an external side effect actually happened.
    ...
    def _task_owner_id(self) -> Optional[str]:
        """Return only the server-provided owner for Web task operations.

        The model can choose a task id but must never choose the principal
        used to authorize it. Non-Web channels keep their historical shared
        scheduling behavior until they have an equivalent authenticated owner
        model; Web requests fail closed when that trusted value is absent.
        """
        context = self.current_context
        if not context:
            raise PermissionError("无法获取当前对话上下文")
        owner_id = context.get("session_owner_id")
        if owner_id:
            return str(owner_id)
        channel_type = str(
            context.get("channel_type")
            or self.config.get("channel_type")
            or ""
        ).lower()
        if channel_type == "web":
            raise PermissionError("Web 定时任务缺少可信所有者")
        return None
    ...
    def _create_task(self, **kwargs) -> str:
        ...
        creator_owner_id = self._task_owner_id() or ""
    ...
    def _list_tasks(self, **kwargs) -> str:
        """List all tasks"""
        tasks = self.task_store.list_tasks(owner_id=self._task_owner_id())
    ...
    def _get_task(self, **kwargs) -> str:
        ...
        task = self.task_store.get_task(task_id, owner_id=self._task_owner_id())
        if not task:
    ...
    def _delete_task(self, **kwargs) -> str:
        ...
        owner_id = self._task_owner_id()
        task = self.task_store.get_task(task_id, owner_id=owner_id)
        if not task:
            return f"错误: 任务 '{task_id}' 不存在"
        
        self.task_store.delete_task(task_id, owner_id=owner_id)
    ...
    def _enable_task(self, **kwargs) -> str:
        ...
        self.task_store.enable_task(task_id, True, owner_id=owner_id)
    ...
    def _disable_task(self, **kwargs) -> str:
        ...
        self.task_store.enable_task(task_id, False, owner_id=owner_id)
