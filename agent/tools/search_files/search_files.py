...
class SearchFiles(BaseTool):
        ...
        for dirpath, dirnames, filenames in os.walk(walk_root):
            ...
            for d in dirnames:
                if d in _SKIP_DIR_NAMES and not no_ignore:
                    continue
                candidate_dir = os.path.join(dirpath, d)
                if self._is_credential_path(candidate_dir):
                    continue
                if is_machine_managed_knowledge_path(
                    candidate_dir, self.cwd
                ) or is_governed_private_path(candidate_dir, self.cwd):
                    continue
                kept.append(d)
            ...
            for filename in filenames:
                ...
                if self._is_credential_path(full):
                    continue
                if is_machine_managed_knowledge_path(full, self.cwd):
                    continue
                if is_governed_private_path(full, self.cwd):
                    continue
                try:
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        if is_machine_managed_knowledge_path(root, self.cwd):
            return ToolResult.fail(
                "Error: governed knowledge must be accessed through "
                "knowledge_search or knowledge_get"
            )
        if is_governed_private_path(root, self.cwd):
            return ToolResult.fail(
                "Error: governed private data must be searched through its dedicated search tool"
            )
        ...
        # 外部后端可能在 no_ignore=true 时遍历隐藏目录，统一在结果层再次过滤。
        outcome.rows = [
            row
            for row in outcome.rows
            if not self._row_targets_managed_path(row, root)
        ]
    ...
    def _row_targets_managed_path(self, row: dict, root: str) -> bool:
        """把后端返回的相对路径还原后检查机器管理目录。"""
        ...
        return is_machine_managed_knowledge_path(
            file_path, self.cwd
        ) or is_governed_private_path(file_path, self.cwd)
    ...
    def _parse_lines(self, lines: List[str], opts: "_SearchOptions") -> List[dict]:
        ...
        for line in lines:
            ...
            if opts.output_mode == "count":
                ...
                continue
            # 内容模式使用正则定位数字行号，避免把 Windows 盘符 C: 当成分隔符。
            match = re.match(r"^(.*?):([0-9]+):(.*)$", line)
            if match is None:
                continue
            file_part, line_no, content = match.groups()
        ...
        for dirpath, dirnames, filenames in os.walk(walk_root):
            ...
            for filename in sorted(filenames):
                ...
                if self._is_credential_path(fp):
                    continue
                if is_machine_managed_knowledge_path(fp, self.cwd):
                    continue
                if is_governed_private_path(fp, self.cwd):
                    continue
