...
- For destructive commands out of workspace ({{cwd}}), explain and confirm first"""
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        # 通用命令执行器不能直接触碰机器管理的记忆或知识路径。
        managed_path_pattern = re.compile(
            r"(?i)(?:MEMORY\.md|governance\.db|memory[/\\]\.governed(?:[/\\]|$)|"
            r"(?:^|[\s\"'=:/\\])knowledge(?:[/\\]|$))"
        )
        if managed_path_pattern.search(command):
            return ToolResult.fail(
                "Error: machine-managed memory or knowledge must be accessed through dedicated lifecycle tools."
            )
        ...
        try:
            ...
            if run_in_background:
                ...
                owner_id, session_id = self._background_scope()
                job_id = background.start(
                    command,
                    self.cwd,
                    env,
                    temp_script_path,
    ...
    def _background_followup(self, bash_id: str, want_kill: bool) -> ToolResult:
        ...
        owner_id, session_id = self._background_scope()
        if want_kill:
            if background.kill(bash_id, owner_id=owner_id, session_id=session_id) is None:
        ...
        state = background.read(bash_id, owner_id=owner_id, session_id=session_id)
        if state is None:
            return ToolResult.fail(self._unknown_job_message(bash_id))
    ...
    def _background_scope(self) -> Tuple[object, object]:
        ...
        return (
            getattr(agent, "conversation_owner_id", None),
            getattr(agent, "session_id", None),
        )

    def _unknown_job_message(self, bash_id: str) -> str:
        owner_id, session_id = self._background_scope()
        jobs = background.list_jobs(owner_id=owner_id, session_id=session_id)
        if not jobs:
