...
def _is_authorized(
    job: _Job, owner_id: Optional[str], session_id: Optional[str]
) -> bool:
    """Keep Web-owned process ids, commands and output session-private."""
    if job.owner_id is None and job.session_id is None:
        return owner_id is None and session_id is None
    return job.owner_id == owner_id and job.session_id == session_id
...
def read(
    ...
    if job is None or not _is_authorized(job, owner_id, session_id):
        return None
...
def list_jobs(
    ...
    with _lock:
        jobs = [
            ...
            if _is_authorized(job, owner_id, session_id)
        ]
    return [
        {
            "id": j.id,
