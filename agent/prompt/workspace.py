...
- **首次写入时**：先用 `knowledge_search` 检查重复内容，再通过 `knowledge_write` 选择合适的分类路径
...
知识页面和 `knowledge/index.md` 是事实库生成的只读投影；写入使用 `knowledge_write`，检索使用 `knowledge_search` 并逐字保留返回的完整 `knowledge://` 引用。禁止截断、调整参数顺序或手工重建引用；失效时必须重新检索。
...
- **On first write**: use `knowledge_search` to check for duplicates, then choose a suitable category path with `knowledge_write`
...
Knowledge pages and `knowledge/index.md` are read-only projections generated from the fact store; use `knowledge_write` for changes and preserve each complete `knowledge://` citation from `knowledge_search` verbatim. Never shorten, reorder, or reconstruct a citation; search again when one is rejected.
