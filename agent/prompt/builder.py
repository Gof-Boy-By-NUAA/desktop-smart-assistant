...
def _build_memory_section(memory_manager: Any, tools: Optional[List[Any]], language: str) -> List[str]:
    ...
    if tools:
        ...
        has_memory_tools = all(
            name in tool_names
            for name in ['memory_search', 'memory_get', 'memory_write']
        )
    ...
    if language == "en":
        lines = [
            ...
            "No need to re-search if the information is already in the loaded legacy MEMORY.md snapshot. Governed memory content must be retrieved via tools.",
            "",
            "1. Location unknown → `memory_search` (keyword / semantic search)",
            "2. Governed memory ID known → `memory_get` with `memory_id`",
            "3. Legacy file location known → `memory_get` to read the exact lines",
            "",
            "**Memory file structure**:",
            "- Governed records: immutable versions with audit, revoke, rollback, and access control",
            "- `MEMORY.md`: read-only legacy snapshot already loaded into context",
            f"- `memory/YYYY-MM-DD.md`: legacy daily memory; today is `memory/{today_file}`",
            "- `knowledge/`: structured knowledge base (see the knowledge system below)",
            "",
            "### Writing memory",
            "",
            "In the following cases, **proactively** use `memory_write` (no need to expose implementation details):",
            "",
            ...
            "",
            "**Lifecycle rules**:",
            "- Durable preference/fact/decision → `memory_write` with `scope=user`",
            "- Session-only event/progress → `memory_write` with `scope=session`",
            "- Correct an existing record → `memory_write` with its `memory_id`",
            "- Forget an active record → `memory_revoke`; restore history only on explicit request → `memory_rollback`",
            "- Structured knowledge → `knowledge/` (see the knowledge system)",
            "- Never use `write` or `edit` on `MEMORY.md` or `memory/.governed/`",
            "- `source_ref` and `evidence_quote` are source claims; do not describe them as independently verified evidence",
        ...
        lines = [
            ...
            "如果已加载的旧版 MEMORY.md 快照中已有相关信息则无需重复检索。治理记忆正文必须通过工具读取。",
            "",
            "1. 不确定位置 → `memory_search` 关键词/语义检索",
            "2. 已知治理记忆 ID → `memory_get` 传入 `memory_id`",
            "3. 已知旧文件位置 → `memory_get` 读取对应行",
            "",
            "**记忆文件结构**:",
            "- 治理记录：具备不可变版本、审计、撤销、回滚和访问控制",
            "- `MEMORY.md`: 已加载到上下文的只读旧版快照",
            f"- `memory/YYYY-MM-DD.md`: 旧版每日记忆，今天是 `memory/{today_file}`",
            "- `knowledge/`: 结构化知识库（见下方知识系统）",
            "",
            "### 写入记忆",
            "",
            "遇到以下情况时，**主动**使用 `memory_write`（无需暴露实现细节）：",
            "",
            ...
            "",
            "**生命周期规则**:",
            "- 长期偏好、事实、决策 → `memory_write`，`scope=user`",
            "- 仅本会话有效的事件、进展 → `memory_write`，`scope=session`",
            "- 更正既有记录 → `memory_write` 并传入原 `memory_id`",
            "- 忘记有效记录 → `memory_revoke`；只有明确要求恢复时才用 `memory_rollback`",
            "- 结构化知识 → `knowledge/`（见知识系统）",
            "- 禁止用 `write` 或 `edit` 修改 `MEMORY.md` 和 `memory/.governed/`",
            "- `source_ref` 和 `evidence_quote` 只是来源声明，不得表述为已经独立核验的证据",
            "- **禁止写入敏感信息**（API密钥、令牌等）",
            "",
...
def _build_knowledge_section(workspace_dir: str, language: str) -> List[str]:
    ...
    if language == "en":
        lines = [
            ...
            "",
            "1. **User shares an article / link / document** → after reading and understanding, call `knowledge_write` for `sources/<slug>.md` in the same turn",
            "2. **An in-depth discussion produces a conclusion / plan** → call `knowledge_write` for `analysis/<slug>.md`",
            "3. **The conversation involves an important entity** (person / company / project) → search first, then create or update it through `knowledge_write`",
            "4. **A technical concept / methodology is discussed** → call `knowledge_write` for `concepts/<topic>.md`",
            "",
            "Use `knowledge_search` for retrieval and preserve every returned `knowledge://` citation verbatim when grounding an answer.",
            "Never shorten, reorder, or manually reconstruct a citation URI. If `knowledge_get` rejects it, run `knowledge_search` again and use the newly returned URI.",
            "Knowledge Markdown files and `knowledge/index.md` are read-only projections; never modify them with file, edit, or shell tools.",
        ...
        lines = [
            ...
            "",
            "1. **用户分享了文章/链接/文档** → 阅读理解后，在同一轮调用 `knowledge_write` 写入 `sources/<slug>.md`",
            "2. **深度讨论产生了结论/方案** → 调用 `knowledge_write` 写入 `analysis/<slug>.md`",
            "3. **对话涉及重要实体**（人物/公司/项目）→ 先检索，再通过 `knowledge_write` 创建或更新",
            "4. **讨论了技术概念/方法论** → 调用 `knowledge_write` 写入 `concepts/<topic>.md`",
            "",
            "检索必须使用 `knowledge_search`，基于结果回答时逐字保留返回的完整 `knowledge://` 引用。",
            "禁止截断、调整参数顺序或手工重建引用 URI；若 `knowledge_get` 拒绝引用，应重新调用 `knowledge_search` 并使用新返回的 URI。",
            "知识 Markdown 和 `knowledge/index.md` 是只读投影，禁止使用文件、编辑或命令行工具改写。",
    ...
    lines.extend([
        ("**How to query**: use `knowledge_search`, then use `knowledge_get` with a returned `knowledge://` citation when more context is needed."
         if language == "en" else
         "**查询方式**：先用 `knowledge_search`，需要更多上下文时再把返回的 `knowledge://` 引用交给 `knowledge_get`。"),
        "",
    ])
...
def _build_workspace_section(workspace_dir: str, language: str) -> List[str]:
    ...
    if language == "en":
        lines = [
            ...
            "- ✅ `MEMORY.md`: loaded - read-only legacy memory snapshot",
            "",
            "**💬 Communication norms**:",
            "",
