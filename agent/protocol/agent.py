...
class Agent:
    ...
    def get_skills_prompt(self, skill_filter=None) -> str:
        ...
        try:
            with self._skill_prompt_lock():
                if hasattr(self.skill_manager, "set_identity_context"):
                    self.skill_manager.set_identity_context(
                        self.identity_context
                    )
                if hasattr(self.skill_manager, "refresh_skills"):
                    self.skill_manager.refresh_skills()
                resolved_filter = self._resolve_explicit_skill_filter(
                    skill_filter
                )
                return self.skill_manager.build_skills_prompt(
                    skill_filter=resolved_filter
                )
        except Exception as e:
    ...
    def _resolve_explicit_skill_filter(self, skill_filter):
        """复核显式请求的治理技能，失败时只保留非治理技能。"""

        if skill_filter is None or not self.skill_manager:
            return skill_filter
        verifier = getattr(
            self.skill_manager, "verify_explicit_skill_filter", None
        )
        if verifier is None:
            return skill_filter
        try:
            allowed, _verified = verifier(
                self.identity_context,
                skill_filter,
                self._skill_model_id(),
            )
            return allowed
        except Exception as error:
            logger.warning(
                "[Agent] Explicit governed skill filter failed closed: %s",
                type(error).__name__,
            )
            fallback = getattr(
                self.skill_manager,
                "explicit_non_governed_skill_filter",
                None,
            )
            if fallback is None:
                return []
            try:
                return fallback(skill_filter)
            except Exception:
                return []

    def _skill_prompt_lock(self):
        """返回覆盖技能刷新、复核和提示词构建的治理锁。"""

        from contextlib import nullcontext

        factory = getattr(
            self.skill_manager, "production_injection_lock", None
        )
        return factory() if factory is not None else nullcontext()

    @staticmethod
    def _strip_skill_sections(prompt: str) -> str:
        """从缓存提示词移除所有已知格式的技能元数据区段。"""

        import re

        sanitized = re.sub(
            r"\s*<(?:available|unavailable)_skills>.*?</(?:available|unavailable)_skills>",
            "",
            str(prompt or ""),
            flags=re.DOTALL,
        )
        return re.sub(
            r"(?ms)^## \U0001f9e9 (?:Skills|技能系统).*?(?=^## |\Z)",
            "",
            sanitized,
        )
    ...
    def get_full_system_prompt(
        ...
        preverified_skill_filter: bool = False,
    ) -> str:
        """
        ...
        try:
            ...
            with self._skill_prompt_lock():
                if self.skill_manager:
                    ...
                    self.skill_manager.refresh_skills()
                    if not preverified_skill_filter:
                        skill_filter = self._resolve_explicit_skill_filter(
                            skill_filter
                        )

                context_files = (
                    load_context_files(self.workspace_dir)
                ...
                builder = PromptBuilder(
                    workspace_dir=self.workspace_dir or "", language=lang
                )
                full = builder.build(
                    tools=self.tools,
            ...
            if strict_skill_filter and skill_filter is not None:
                raise
            fallback = self.system_prompt
            if self.extra_system_suffix:
                fallback = f"{fallback}\n\n{self.extra_system_suffix}"
            if self.skill_manager:
                fallback = self._strip_skill_sections(fallback)
            return fallback
