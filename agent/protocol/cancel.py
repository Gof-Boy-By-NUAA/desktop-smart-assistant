...
class _CancelEntry:
    __slots__ = ("event", "session_id", "owner_id")

    def __init__(self, session_id: Optional[str], owner_id: Optional[str]):
...
class CancelTokenRegistry:
    ...
    def register(
        ...
        with self._lock:
            ...
            if entry is None:
                entry = _CancelEntry(session_id, owner_id)
                self._by_request[request_id] = entry
                if session_id:
                    self._by_session.setdefault(session_id, set()).add(request_id)
            elif entry.owner_id != owner_id or entry.session_id != session_id:
                raise PermissionError("cancel token ownership mismatch")
            return entry.event
    ...
    def cancel_request_owned(self, request_id: str, owner_id: str) -> bool:
        """Cancel only when the request was registered for this owner."""
    ...
    def cancel_session_owned(self, session_id: str, owner_id: str) -> int:
        """Cancel in-flight requests for one session and one trusted owner."""
        ...
        with self._lock:
            ...
            entries = [
                ...
                and self._by_request[request_id].owner_id == owner_id
            ]
