/* =====================================================================
   SmartAssistant Console - Main Application Script
   ===================================================================== */
...
function applyI18n() {
    ...
    if (docsLink) docsLink.href = currentLang === 'zh' ? 'https://docs.smart_assistant.ai/zh' : 'https://docs.smart_assistant.ai';
...
function createMd() {
    ...
    var KNOWLEDGE_CITATION_RE = /knowledge:\/\/[^\s<>"']+?&citation_version=3/g;
    md.core.ruler.after('fix_greedy_linkify', 'link_governed_citations', function(state) {
        for (var b = 0; b < state.tokens.length; b++) {
            var block = state.tokens[b];
            if (block.type !== 'inline' || !block.children) continue;
            var output = [];
            for (var i = 0; i < block.children.length; i++) {
                var token = block.children[i];
                if (token.type !== 'text' || token.content.indexOf('knowledge://') < 0) {
                    output.push(token);
                    continue;
                }
                var cursor = 0;
                var matches = token.content.matchAll(KNOWLEDGE_CITATION_RE);
                for (var match of matches) {
                    var index = match.index || 0;
                    if (index > cursor) {
                        var prefix = new state.Token('text', '', 0);
                        prefix.content = token.content.slice(cursor, index);
                        output.push(prefix);
                    }
                    var citationOpen = new state.Token('link_open', 'a', 1);
                    citationOpen.attrs = [
                        ['href', match[0]],
                ...
                if (cursor < token.content.length) {
                    var suffix = new state.Token('text', '', 0);
...
fetch('/config').then(r => r.json()).then(data => {
    if (data.status === 'success') {
        appConfig = data;
        const title = data.title || 'SmartAssistant';
...
async function resolveKnowledgeCitation(uri, anchor) {
    var host = anchor.closest('.agent-content-body, .answer-content, .msg-content') || anchor.parentElement;
    if (!host) return;
    var existing = host.querySelector('.knowledge-citation-card');
    if (existing) existing.remove();

    var card = document.createElement('section');
    card.className = 'knowledge-citation-card';
    ...
    try {
        var response = await fetch('/api/knowledge/citation/resolve', {
            method: 'POST',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (result.status !== 'success' || !result.citation) {
            ...
            retry.type = 'button';
            retry.textContent = 'Retry verification / 重新验证';
            retry.style.cssText = 'display:block;margin-top:8px;padding:4px 8px;border:1px solid currentColor;border-radius:6px;';
            retry.addEventListener('click', function() { resolveKnowledgeCitation(uri, anchor); });
        ...
        source.textContent = citation.source_ref;
        var meta = document.createElement('div');
        meta.style.cssText = 'margin-top:4px;opacity:.7;';
        meta.textContent = 'v' + citation.document_version + ' · bytes ' + citation.byte_start + '–' + citation.byte_end;
        var quote = document.createElement('blockquote');
        quote.style.cssText = 'margin:8px 0 0;padding-left:8px;border-left:2px solid var(--primary-color,#2563eb);white-space:pre-wrap;';
...
messagesDiv.addEventListener('click', (e) => {
    ...
    if (href.startsWith('knowledge://')) {
        e.preventDefault();
        e.stopPropagation();
        resolveKnowledgeCitation(href, a);
    } else if (href === '/memory/dreams') {
        e.preventDefault();
        navigateTo('memory');
...
function requestCancel() {
    ...
    if (!reqId || cancelRequestPending) return;
    cancelRequestPending = true;
    sendBtn.disabled = true;
    sendBtn.title = (currentLang === 'zh' ? '???????' : 'Requesting cancellation?');
    fetch('/cancel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(async response => {
        ...
        // cancelled label is rendered exclusively by the bound SSE event.
        sendBtn.title = (currentLang === 'zh' ? '???????' : 'Cancellation requested');
    })
    .catch(err => {
        console.warn('[cancel] request was not confirmed', err);
        sendBtn.title = (currentLang === 'zh' ? '?????' : 'Cancellation unconfirmed');
        alert(currentLang === 'zh'
            ? '?????????????????????????'
            : 'Cancellation was not confirmed. The task may still be running; check the connection and retry.');
    })
    .finally(() => {
    ...
    function processSSEItem(item) {
            if (item.type === 'reasoning') {
                ...
                if (toolEl) {
                    ...
                    const isStructuredSummary = item.result_transport?.truncated === true;
                    const displayResult = formatToolResultForDisplay(
                        item.result,
                        item.result_transport
                    );
                    ...
                    if (outputLabel) {
                        outputLabel.textContent = isError
                            ? 'Error'
                            : (isStructuredSummary
                                ? (currentLang === 'zh' ? '结构化摘要' : 'Structured summary')
                                : 'Output');
                    }
                    if (outputEl) {
                        outputEl.textContent = displayResult;
...
function newChat(optimistic = true) {
    ...
    ws.innerHTML = `
        <img src="assets/logo.jpg" alt="SmartAssistant" class="w-16 h-16 rounded-2xl mb-6 shadow-lg shadow-primary-500/20">
        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3">${appConfig.title || 'SmartAssistant'}</h1>
...
function stableToolResultValue(value) {
    if (Array.isArray(value)) {
        return value.map(stableToolResultValue);
    }
    if (value !== null && typeof value === 'object') {
        const sorted = Object.create(null);
        Object.keys(value).sort().forEach(key => {
            sorted[key] = stableToolResultValue(value[key]);
        });
        return sorted;
    }
    return value;
...
function stableStringifyToolResult(value) {
    return JSON.stringify(stableToolResultValue(value), null, 2);
}

function formatToolResultForDisplay(result, transport = null) {
    let resultText = '';
    if (typeof result === 'string') {
        resultText = result;
    } else if (result !== undefined && result !== null) {
        if (typeof result === 'object') {
            resultText = stableStringifyToolResult(result);
        } else {
            resultText = String(result);
        }
    }

    if (!transport || transport.truncated !== true) {
        return resultText;
    }

    const originalBytes = Number.isFinite(transport.original_json_bytes)
        ? transport.original_json_bytes
        : '?';
    const citationCount = Number.isFinite(transport.preserved_citation_count)
        ? transport.preserved_citation_count
        : 0;
    const notice = currentLang === 'zh'
        ? `[结果过大，已显示结构化摘要；原始 ${originalBytes} 字节，保留 ${citationCount} 条完整引用]`
        : `[Result exceeded the display limit; showing a structured summary of ${originalBytes} bytes with ${citationCount} complete citations preserved]`;
...
const WECOM_BOT_SOURCE = 'smart_assistant';
...
function initApp() {
    ...
    fetch('/api/version').then(r => r.json()).then(data => {
        ...
        document.getElementById('sidebar-version').textContent = `SmartAssistant ${APP_VERSION}`;
    }).catch(() => {
        document.getElementById('sidebar-version').textContent = 'SmartAssistant';
    });
    chatInput.focus();
}
...
fetch('/auth/check').then(r => r.json()).then(data => {
    ...
    // Authentication uncertainty must never be treated as authorization. Keep
    // the application hidden and expose a recoverable connection error.
    showLoginScreen();
    const errEl = document.getElementById('login-error');
    if (errEl) {
        errEl.textContent = currentLang === 'zh'
            ? '???????????????????'
            : 'Authentication status unavailable; check the backend and retry';
        errEl.classList.remove('hidden');
    }
});
...
function saveTaskEdit() {
    ...
    // Receiver, notify_session_id, channel and delivery metadata are server-
    // owned. Submit only the explicitly editable action fields.
