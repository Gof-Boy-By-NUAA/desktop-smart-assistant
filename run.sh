...
# ============================
# SmartAssistant Management Script
# ============================
...
# Clone project
clone_project() {
    echo -e "${GREEN}🔍 Cloning SmartAssistant project...${NC}"

    if [ -d "SmartAssistant" ]; then
        # An existing directory is automatically backed up (no prompt) so the
        # installer stays one-shot / hands-off.
        local backup_dir="SmartAssistant_backup_$(date +%s)"
        echo -e "${YELLOW}⚠️  $(t "目录 'SmartAssistant' 已存在，自动备份到" "Directory 'SmartAssistant' exists, backing up to") '$backup_dir'...${NC}"
        mv SmartAssistant "$backup_dir"
    fi
    ...
    if ! command -v git &> /dev/null; then
        ...
        local zip_url="https://gitee.com/zhayujie/SmartAssistant/repository/archive/master.zip"
        if command -v wget &> /dev/null; then
            wget "$zip_url" -O SmartAssistant.zip
        elif command -v curl &> /dev/null; then
            curl -L "$zip_url" -o SmartAssistant.zip
        else
        ...
        if command -v unzip &> /dev/null; then
            unzip SmartAssistant.zip
        elif command -v python3 &> /dev/null; then
            python3 -m zipfile -e SmartAssistant.zip .
        elif command -v python &> /dev/null; then
            python -m zipfile -e SmartAssistant.zip .
        else
            echo -e "${RED}❌ Cannot extract archive. Please install 'unzip' or Python.${NC}"
            exit 1
        fi
        # Archive top-level dir name may vary (SmartAssistant-master, etc.); detect it.
        local _extracted="SmartAssistant-master"
        if [ ! -d "$_extracted" ]; then
            _extracted=$(ls -d SmartAssistant-*/ 2>/dev/null | head -1 | sed 's:/*$::')
        fi
        [ -n "$_extracted" ] && [ -d "$_extracted" ] && mv "$_extracted" SmartAssistant
        rm -f SmartAssistant.zip
    else
        ...
        if curl -sI --connect-timeout 5 --max-time 10 https://github.com > /dev/null 2>&1; then
            ...
            _timeout 60 git clone --depth 10 --progress https://github.com/zhayujie/SmartAssistant.git && clone_ok=true
        fi
        if [ "$clone_ok" = false ]; then
            echo -e "${YELLOW}⚠️  GitHub clone failed or timed out, switching to Gitee mirror...${NC}"
            _timeout 30 git clone --depth 10 --progress https://gitee.com/zhayujie/SmartAssistant.git && clone_ok=true
        fi
        if [ "$clone_ok" = false ]; then
    ...
    cd SmartAssistant || { echo -e "${RED}❌ Failed to enter project directory.${NC}"; exit 1; }
    export BASE_DIR=$(pwd)
...
start_project() {
    echo ""
    echo -e "${GREEN}${EMOJI_ROCKET} Starting SmartAssistant...${NC}"
    sleep 1
    ...
    if $USE_COW; then
        ...
        if [[ "$OS_TYPE" == "Linux" ]]; then
            ...
            echo -e "${GREEN}${EMOJI_COW} SmartAssistant started on Linux (using $PYTHON_CMD)${NC}"
        elif [[ "$OS_TYPE" == "Darwin" ]]; then
            nohup $PYTHON_CMD "${BASE_DIR}/app.py" > "${BASE_DIR}/nohup.out" 2>&1 &
            echo -e "${GREEN}${EMOJI_COW} SmartAssistant started on macOS (using $PYTHON_CMD)${NC}"
        else
    ...
    echo ""
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${GREEN}${EMOJI_CHECK} $(t "SmartAssistant 已在后台运行" "SmartAssistant is now running in background")!${NC}"
...
show_usage() {
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_COW} SmartAssistant Management Script${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
cmd_start() {
    ...
    if has_cow; then
        ...
        if is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} $(t "SmartAssistant 已在运行中" "SmartAssistant is already running") (PID: $(get_pid))${NC}"
...
cmd_stop() {
    ...
    if has_cow; then
        ...
        echo -e "${GREEN}${EMOJI_STOP} $(t "正在停止 SmartAssistant" "Stopping SmartAssistant")...${NC}"

        if ! is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} $(t "SmartAssistant 未在运行" "SmartAssistant is not running")${NC}"
            return 0
        fi
        ...
        echo -e "${GREEN}${EMOJI_CHECK} $(t "SmartAssistant 已停止" "SmartAssistant stopped")${NC}"
    fi
}
...
cmd_status() {
    if has_cow; then
        ...
        echo -e "${CYAN}${BOLD}=========================================${NC}"
        echo -e "${CYAN}${BOLD}   ${EMOJI_COW} SmartAssistant Status${NC}"
        echo -e "${CYAN}${BOLD}=========================================${NC}"
...
cmd_config() {
    ...
    select_language
    echo ""
    echo -e "${YELLOW}${EMOJI_WRENCH} $(t "正在重新配置 SmartAssistant" "Reconfiguring SmartAssistant")...${NC}"
...
# Update project
cmd_update() {
    echo -e "${GREEN}${EMOJI_WRENCH} $(t "正在更新 SmartAssistant" "Updating SmartAssistant")...${NC}"
    cd "${BASE_DIR}"
    ...
    if [ -d .git ]; then
        ...
        if git pull; then
            ...
            git remote set-url origin https://gitee.com/zhayujie/SmartAssistant.git
            if git pull; then
                pull_ok=true
            else
...
install_mode() {
    ...
    clear
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_COW} SmartAssistant Installation${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
