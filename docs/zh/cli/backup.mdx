---
title: 备份与恢复
description: 导出和恢复 SmartAssistant 配置及 Agent 工作区数据
---

SmartAssistant 可以生成便携的本地归档，用于设备迁移或灾难恢复。
...
恢复命令会在写入前校验归档格式和所有路径。它会覆盖同名文件，但不会删除目标目录中无关的现有文件。如果检测到当前 SmartAssistant 数据，会先在所选归档旁创建 `cow-pre-restore-*.zip` 回滚备份。
