---
title: Web 控制台
description: 通过 Web 控制台使用 SmartAssistant
---

Web 控制台是 SmartAssistant 的默认通道，启动后会自动运行，通过浏览器即可与 Agent 对话，并支持在线管理模型、技能、记忆、通道等配置。
...
| `web_host` | 内置 Web 服务监听地址，强制为回环地址；如需对外提供服务，必须通过 TLS 反向代理 | `127.0.0.1` |
| `web_port` | Web 服务监听端口 | `9899` |
| `web_password` | 内置控制台访问密码 | `""` |
...
- 服务器运行：使用 TLS 反向代理转发到 `127.0.0.1:9899` 的 HTTPS 地址；不要直接暴露内置 HTTP 监听器
