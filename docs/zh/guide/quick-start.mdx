---
title: 一键安装
description: 使用脚本一键安装和管理 SmartAssistant
---
...
<Note>
  **服务器部署需要对外提供控制台时**，内置服务会强制监听 `127.0.0.1`，禁止直接暴露 HTTP 端口。请在 `127.0.0.1:9899` 前配置 TLS 反向代理，在代理层执行防火墙/安全组策略，并将 `web_password` 作为纵深防御。
</Note>
