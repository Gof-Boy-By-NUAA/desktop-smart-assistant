---
title: 手动安装
description: 手动部署 SmartAssistant（源码 / Docker）
---
...
<Tip>
  若遇到网络问题可使用国内仓库地址：https://gitee.com/zhayujie/SmartAssistant
</Tip>
...
<Tip>
  **服务器对外提供 Web 控制台**：内置服务会强制监听 `127.0.0.1`，禁止直接暴露 HTTP 端口。请在 `127.0.0.1:9899` 前配置 TLS 反向代理，在代理层执行防火墙/安全组策略，并将 `web_password` 作为纵深防御。
</Tip>
...
<Tip>
  **Docker 对外提供 Web 控制台**：保持内置服务回环监听，在可信主机或反向代理 sidecar 中终止 TLS。不要直接发布内置 HTTP 端口；将 `WEB_PASSWORD` 作为纵深防御。
</Tip>
...
| `agent_workspace` | - | Agent 工作空间路径 | `./workspace` |
...
<Tip>
  全部配置项可在项目 [`config.py`](https://github.com/zhayujie/SmartAssistant/blob/master/config.py) 文件中查看。Docker 部署时，配置项名称需转为大写环境变量格式。
</Tip>
