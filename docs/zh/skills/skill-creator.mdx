...
| `metadata.smart_assistant.requires.bins` | 系统里必须装了哪些命令行工具 |
| `metadata.smart_assistant.requires.env` | 需要哪些环境变量（全部满足才行） |
| `metadata.smart_assistant.requires.anyEnv` | 多个 API Key 满足一个就行 |
| `metadata.smart_assistant.requires.anyBins` | 多个工具满足一个就行 |
| `metadata.smart_assistant.always` | 设为 `true` 会始终加载，不检查依赖 |
| `metadata.smart_assistant.emoji` | 展示用的 emoji（可选） |
| `metadata.smart_assistant.os` | 限定系统，如 `["darwin", "linux"]` |
