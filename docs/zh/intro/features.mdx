---
title: 功能介绍
description: SmartAssistant 长期记忆、个人知识库、任务规划、技能系统、CLI 命令、浏览器工具详细说明
---
...
第一次启动 Agent 时，Agent 会主动询问关键信息，并记录至工作空间（默认 `./workspace`）中的智能体设定、用户身份、记忆文件中。
...
<Frame>
  <img src="https://cdn.jsdelivr.net/gh/zhayujie/smart-assistant-assets@main/screenshots/zh/web-console-evolution-demo-zh.png" width="800" />
</Frame>
...
Agent 会在对话中自动将有价值的信息整理为知识页面，维护交叉引用和索引，通过 Web 控制台可浏览文档和查看知识图谱。知识库存储在工作空间的 `./workspace/knowledge/` 目录下。
...
- [Skill Hub](https://skills.smart_assistant.ai/)：开放的技能广场，汇集官方推荐、社区贡献和第三方技能，支持一键安装。
- **内置技能：** 在项目的 `skills/` 目录下，包含技能创造器、图像识别、LinkAI 智能体、网页抓取等。内置 Skill 根据依赖条件（API Key、系统命令等）自动判断是否启用。
- **自定义技能：** 由用户通过对话创建，存放在工作空间中（`./workspace/skills/`），可实现任何复杂的业务流程和第三方系统对接。
...
访问 [skills.smart_assistant.ai](https://skills.smart_assistant.ai/) 浏览所有可用技能，或在对话中执行：
...
SmartAssistant 提供两种命令交互方式，覆盖服务管理、技能安装、配置调整等日常运维操作：
...
详细命令参考 [命令总览](https://docs.smart_assistant.ai/cli)。
