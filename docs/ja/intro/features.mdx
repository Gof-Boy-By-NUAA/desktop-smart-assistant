---
title: 機能詳細
description: SmartAssistant の長期記憶、タスク計画、Skill システム、CLI コマンド、ブラウザツールの詳細
---
...
初回起動時、Agent はユーザーに重要な情報を自発的に尋ね、ワークスペース（デフォルト `./workspace`）に記録します。これには Agent の設定、ユーザーの身元情報、記憶ファイルが含まれます。
...
Agent は会話中に価値ある情報を自動的にナレッジページとして整理し、相互参照とインデックスを維持します。Web コンソールではドキュメントの閲覧とナレッジグラフの可視化が可能です。ナレッジはワークスペースの `./workspace/knowledge/` ディレクトリに保存されます。
...
- [Skill Hub](https://skills.smart_assistant.ai/)：オープンな Skill マーケットプレイス。公式推奨、コミュニティ、サードパーティの Skill を収録。ワンコマンドでインストール可能。
...
- **カスタム Skill：** ユーザーが会話を通じて作成し、ワークスペース（`./workspace/skills/`）に保存されます。あらゆる複雑なビジネスプロセスやサードパーティ連携を実装できます。
...
[skills.smart_assistant.ai](https://skills.smart_assistant.ai/) で利用可能なすべての Skill を閲覧するか、会話内でコマンドを実行できます：
...
SmartAssistant はサービス管理、Skill インストール、設定変更などをカバーする2つのコマンドインターフェースを提供します：
...
詳細は [コマンド一覧](https://docs.smart_assistant.ai/ja/cli) を参照してください。
