---
title: Telegram
description: Telegram Bot API 経由で SmartAssistant を接続
---

> 公式の Telegram Bot API を通じて SmartAssistant を接続します。1 対 1 チャットおよびグループチャット（@メンションまたはボットへの返信で起動）に対応。Long Polling 方式のため公開 IP は不要で、すぐに利用できます。
...
2. `/newbot` を送り、案内に従って入力します:
   - **Bot 名**（表示名、例: `My SmartAssistant Bot`）
   - **Bot ユーザー名**（`bot` で終わる必要があります、例: `my_smart_assistant_bot`）
...
- **1 対 1 チャット**: Telegram で Bot のユーザー名（例: `@my_smart_assistant_bot`）を検索し、`Start` をタップして会話を開始します。
