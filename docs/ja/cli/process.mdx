---
title: プロセス管理
description: cow コマンドで SmartAssistant プロセスのライフサイクルを管理
---

プロセス管理コマンドは SmartAssistant バックグラウンドプロセスのライフサイクルを制御します。これらのコマンドはターミナルでのみ使用可能です。

## start

SmartAssistant サービスを起動します。デフォルトではバックグラウンドデーモンとして実行され、自動的にログを表示します。
...
実行中の SmartAssistant サービスを停止します。
...
SmartAssistant サービスを再起動します（停止してから起動）。
...
SmartAssistant サービスの実行状態を確認します。プロセス情報、バージョン、現在のモデルとチャネルの設定を含みます。
