...
description: Download and use the SmartAssistant desktop client (macOS / Windows)
---

SmartAssistant ships a ready-to-use desktop client with the Agent runtime bundled in — **no need to install Python or dependencies manually**. Just download, install, and run your local super AI assistant.
...
<Card title="Go to the download page" icon="download" href="https://smart_assistant.ai/download/">
  Download the macOS / Windows installer
</Card>

<Tabs>
  <Tab title="macOS">
    1. Open the [download page](https://smart_assistant.ai/download/) and pick the build for your chip:
       - Apple Silicon (M1/M2/M3/M4): download the `arm64` build
       - Intel: download the `x64` build
    2. Open the downloaded `.dmg` and drag SmartAssistant into your Applications folder.
    3. Launch SmartAssistant from Launchpad or Applications.
  </Tab>
  <Tab title="Windows">
    1. Open the [download page](https://smart_assistant.ai/download/) and download the Windows installer (`.exe`).
    2. Run the installer and follow the prompts (you can choose the install directory).
    3. Launch SmartAssistant from the desktop or Start menu.
  </Tab>
</Tabs>
...
- **Workspace**: `./workspace` by default, holding chat history, knowledge base, memory, skills, scheduled tasks and other files produced by the Agent.
