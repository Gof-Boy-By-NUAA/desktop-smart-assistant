...
| `metadata.smart_assistant.requires.bins` | System CLI tools that must be installed |
| `metadata.smart_assistant.requires.env` | Required environment variables (all must be present) |
| `metadata.smart_assistant.requires.anyEnv` | Multiple API keys — at least one must be set |
| `metadata.smart_assistant.requires.anyBins` | Multiple tools — at least one must be installed |
| `metadata.smart_assistant.always` | Set to `true` to always load, skipping dependency checks |
| `metadata.smart_assistant.emoji` | Display emoji (optional) |
| `metadata.smart_assistant.os` | OS restriction, e.g. `["darwin", "linux"]` |
