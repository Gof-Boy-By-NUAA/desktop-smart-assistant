---
title: Web Console
description: Use SmartAssistant through the Web Console
---

The Web Console is SmartAssistant's default channel. It runs automatically once started, letting you chat with the Agent in a browser and manage models, skills, memory, channels, and other configuration online.
...
| `web_host` | Embedded Web service listen address. It is forced to loopback; expose it only through a TLS reverse proxy | `127.0.0.1` |
| `web_port` | Web service listen port | `9899` |
| `web_password` | Access password for the embedded console | `""` |
...
- Server: publish a TLS reverse-proxy URL that forwards to `127.0.0.1:9899`; do not expose the embedded HTTP listener directly

<Note>
  The embedded listener is loopback-only. Terminate TLS and enforce network policy in the reverse proxy before making the console reachable to other users.
</Note>
