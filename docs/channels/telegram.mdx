---
title: Telegram
description: Integrate SmartAssistant with Telegram via the Bot API
---

> Integrate SmartAssistant into Telegram via the official Bot API. Supports private chat and group chat (triggered by @mention or replying to the bot). Uses Long Polling — no public IP required, works out of the box.
...
2. Send `/newbot` and follow the prompts:
   - **Bot name** (display name, e.g. `My SmartAssistant Bot`)
   - **Bot username** (must end with `bot`, e.g. `my_smart_assistant_bot`)
...
### Step 3: Connect to SmartAssistant
...
```
[Telegram] Bot logged in as @my_smart_assistant_bot (id=123456789)
...
- **Private chat**: search for your bot username (e.g. `@my_smart_assistant_bot`) in Telegram, click `Start` and chat away.
