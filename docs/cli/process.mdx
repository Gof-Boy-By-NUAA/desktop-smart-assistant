...
description: Manage SmartAssistant process lifecycle with cow commands
---

Process management commands control the SmartAssistant background process. These commands are only available in the terminal.

## start

Start the SmartAssistant service. Runs as a background daemon by default and automatically tails logs.
...
Stop the running SmartAssistant service.
...
Restart the SmartAssistant service (stop then start).
...
Check SmartAssistant service status, including process info, version, and current model/channel configuration.
