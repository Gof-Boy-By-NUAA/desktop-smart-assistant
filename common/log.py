...
def _reset_logger(log):
    ...
    log.propagate = False
    stdout = sys.stdout
    # 保留宿主终端声明的编码，避免 Windows GBK 终端把 UTF-8 字节误解码为乱码。
    if hasattr(stdout, "reconfigure"):
        try:
            stdout.reconfigure(errors="replace", line_buffering=True)
    ...
    try:
        try:
            max_bytes = max(int(os.environ.get("COW_LOG_MAX_BYTES", 10 * 1024 * 1024)), 1024)
        except ValueError:
            max_bytes = 10 * 1024 * 1024
        try:
            backup_count = max(int(os.environ.get("COW_LOG_BACKUP_COUNT", 5)), 1)
        except ValueError:
            backup_count = 5
