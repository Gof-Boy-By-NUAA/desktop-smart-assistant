...
class CloudClient(LinkAIClient):
    ...
    def on_message(self, push_msg: PushMsg):
        ...
        logger.info(
            "receive msg push, session_present=%s, content_chars=%d",
            bool(session_id),
            len(msg_content or ""),
        )
        context = Context()
    ...
    def on_chat(self, data: dict, send_chunk_fn):
        ...
        logger.info(
            "[CloudClient] on_chat: session_present=%s, channel=%s, query_chars=%d",
            bool(session_id),
            channel_type,
            len(query),
        )
    ...
    def _query_history(self, payload: dict) -> dict:
        ...
        if not session_id.startswith("session_"):
            session_id = f"session_{session_id}"
        logger.info(
            "[CloudClient] history query: session_present=%s, page=%s, page_size=%s",
            bool(session_id),
            page,
            page_size,
        )
