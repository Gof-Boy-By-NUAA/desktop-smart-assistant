...
const SessionList: React.FC = () => {
  const { sessions, activeId, loading, error, loadSessions, loadMore, hasMore, setActive, newSession, rename, remove } =
    useSessionStore()
  ...
  return (
    <div className="w-[240px] flex-shrink-0 flex flex-col h-full bg-surface border-r border-default">
      ...
      >
        {error && (
          <div className="mx-1 mt-2 rounded-btn border border-danger/30 bg-danger-soft px-2.5 py-2 text-xs text-danger">
            <p className="break-words">{t('session_load_error')}</p>
            <button
              onClick={() => loadSessions(1)}
              className="mt-1 font-medium underline cursor-pointer"
            >
              {t('session_retry')}
            </button>
          </div>
                ...
                >
                  ...
                  {isEditing ? (
                    ...
                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <IconBtn onClick={(e) => {
                        e.stopPropagation()
                        if (window.confirm(t('session_delete_confirm'))) void remove(s.session_id).catch(() => {})
                      }} title={t('session_delete')} danger>
                        <Trash2 size={13} />
                      </IconBtn>
                    </div>
                  )}
