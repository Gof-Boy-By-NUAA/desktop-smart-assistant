...
const App: React.FC = () => {
  ...
  // the main UI render.
  const [authState, setAuthState] = useState<'checking' | 'need_login' | 'unavailable' | 'ok'>('checking')
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch(() => {
        // Authentication uncertainty is not authorization. Fail closed instead
        // of admitting the user to a UI whose protected calls will all fail.
        if (!cancelled) setAuthState('unavailable')
      })
    return () => {
      cancelled = true
    }
  }, [backend.status, backend.baseUrl, authCheckNonce])
  ...
  const handleLogout = useCallback(async () => {
    try {
      ...
      // Do not leave the previous principal's cached conversations visible
      // while the next login is being established, even if the request failed.
  ...
  if (authState === 'unavailable') {
    return (
      <StatusScreen
        status="error"
        error={t('auth_check_unavailable')}
        onRetry={() => {
          setAuthState('checking')
