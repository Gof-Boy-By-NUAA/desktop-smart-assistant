...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message, onRegenerate, onEdit, onDelete, onMediaLoad }) => {
  ...
  const [citationView, setCitationView] = useState<
    | { state: 'idle' }
    | { state: 'loading'; uri: string }
    | { state: 'resolved'; uri: string; citation: KnowledgeCitation }
    | { state: 'error'; uri: string; message: string }
  >({ state: 'idle' })
  const preview = useWorkspaceStore((s) => s.preview)

  const resolveCitation = async (uri: string) => {
    setCitationView({ state: 'loading', uri })
    try {
      const result = await apiClient.resolveKnowledgeCitation(uri)
      if (result.status === 'success') {
        setCitationView({ state: 'resolved', uri, citation: result.citation })
      } else {
        setCitationView({ state: 'error', uri, message: result.message })
      }
    } catch {
      setCitationView({ state: 'error', uri, message: 'Citation verification failed; retry or search again.' })
    }
  }
    ...
    <div className="group flex gap-3 px-4 sm:px-6 py-2">
      ...
      <div className="flex-1 min-w-0 max-w-[calc(100%-2.5rem)]">
        <div className="inline-block w-full rounded-2xl border border-default bg-surface px-4 py-3">
          ...
          {/* Final answer */}
          {message.content && <Markdown content={message.content} onCitationLink={resolveCitation} />}
            ...
            <section className="mt-3 rounded-xl border border-default bg-surface-2 p-3 text-xs" aria-live="polite">
              ...
              {citationView.state === 'resolved' && (
                <div className="mt-2 space-y-1.5">
                  <div className="text-content-secondary break-all">{citationView.citation.source_ref}</div>
                  <div className="text-content-tertiary">
                    v{citationView.citation.document_version} · bytes {citationView.citation.byte_start}–{citationView.citation.byte_end}
                  </div>
                  <blockquote className="border-l-2 border-accent pl-2 text-content whitespace-pre-wrap">
                    {citationView.citation.quote}
                  </blockquote>
                </div>
              )}
              {citationView.state === 'error' && (
                <div className="mt-2">
                  <div className="text-danger">{citationView.message}</div>
                  <button
                    type="button"
                    onClick={() => void resolveCitation(citationView.uri)}
                    className="mt-2 rounded-md border border-default px-2 py-1 text-content-secondary hover:bg-surface-3"
                  >
          ...
          {message.isCancelPending && (
            <div className="text-xs text-warning mt-1">{t('msg_cancel_requesting')}</div>
          )}
          {message.isCancelUnconfirmed && (
            <div className="text-xs text-danger mt-1">{t('msg_cancel_unconfirmed')}</div>
          )}
          {message.isStreamInterrupted && (
            <div className="text-xs text-danger mt-1">{t('msg_stream_interrupted')}</div>
          )}
