  ...
  if (loadError) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
        <h2 className="text-lg font-semibold text-content mb-2">{t('knowledge_load_error')}</h2>
        <p className="text-sm text-danger max-w-lg break-words mb-5">{loadError}</p>
        <button
          onClick={() => {
            setLoading(true)
            setLoadError(null)
            void apiClient.getKnowledgeList().then((fresh) => {
              setData(fresh)
              const first = firstFile(fresh)
              if (first) void openDoc(first.path, first.title)
            }).catch((e) => setLoadError(e instanceof Error ? e.message : String(e))).finally(() => setLoading(false))
          }}
          className="px-4 py-2 rounded-btn bg-accent text-accent-contrast hover:bg-accent-hover text-sm font-medium cursor-pointer"
        >
          {t('knowledge_retry')}
        </button>
      </div>
        ...
        <div className="flex-1 min-h-0 border-t border-default relative">
          {graphLoading ? (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
              <p className="text-sm text-danger max-w-lg break-words mb-3">{t('knowledge_graph_error')}</p>
              <button onClick={() => void loadGraph()} className="text-sm text-accent underline cursor-pointer">
                {t('knowledge_retry')}
              </button>
            </div>
