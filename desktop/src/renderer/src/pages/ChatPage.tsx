...
const ChatPage: React.FC<ChatPageProps> = ({ baseUrl }) => {
  ...
  const historyError = session?.historyError ?? null
  ...
  const handleDelete = useCallback(
    (msg: ChatMessage) => {
      if (msg.userSeq != null && window.confirm(t('msg_delete_confirm'))) {
        void deleteMessage(activeId, msg.userSeq, true)
      }
    },
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto" onScroll={handleScroll}>
        {historyError && (
          <div className="mx-auto mt-3 max-w-3xl px-6">
            <div className="flex items-center justify-between gap-3 rounded-btn border border-danger/30 bg-danger-soft px-3 py-2 text-xs text-danger">
              <span className="break-words">{t('chat_history_failed')}</span>
              <button
                onClick={() => void loadHistory(activeId, 1)}
                className="flex-shrink-0 font-medium underline cursor-pointer"
              >
                {t('chat_retry')}
              </button>
            </div>
        ...
        {isEmpty && historyLoading ? (
          <div className="flex items-center justify-center h-full text-content-tertiary text-sm">
            ...
            {t('chat_loading_history')}
          </div>
        ) : isEmpty ? (
          <div data-home className="chat-home flex flex-col items-center justify-center h-full px-6 py-12">
            <img src="./logo.jpg" alt="SmartAssistant" className="w-16 h-16 rounded-2xl mb-5 shadow-md" />
