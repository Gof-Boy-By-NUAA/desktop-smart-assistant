...
class AgentBridge:
    ...
    def __init__(self, bridge: Bridge):
        ...
        # A session owns mutable Agent state (messages, tools, cancel/progress
        # callbacks). Serialize whole turns per session rather than relying on
        # a channel-specific queue, because Web, cloud and scheduler paths all
        # reach AgentBridge directly.
        self._session_run_locks = {}
    ...
    def _acquire_session_run_lock(
        self, session_id: Optional[str], timeout: Optional[float] = None
    ):
        """Acquire a ref-counted lock for one mutable conversation state."""
        if not session_id:
            return None
        with self._session_run_locks_guard:
            entry = self._session_run_locks.get(session_id)
            if entry is None:
                entry = {"lock": threading.Lock(), "users": 0}
                self._session_run_locks[session_id] = entry
            entry["users"] += 1
        acquired = (
            entry["lock"].acquire()
            if timeout is None
            else entry["lock"].acquire(timeout=max(0.0, timeout))
        )
        if not acquired:
            with self._session_run_locks_guard:
                entry["users"] -= 1
                if (
                    entry["users"] == 0
                    and self._session_run_locks.get(session_id) is entry
                ):
                    self._session_run_locks.pop(session_id, None)
            return None
        return entry

    def _release_session_run_lock(self, session_id: Optional[str], entry) -> None:
    ...
    def cancel_and_wait_for_session(
        self,
        session_id: str,
        owner_id: Optional[str] = None,
        timeout: float = 5.0,
    ) -> bool:
        """Cancel all work for a session, then acquire its mutation fence.

        The returned success means there is no active or queued AgentBridge
        turn that can write after a caller clears/deletes the durable session.
        A timeout is deliberately fail-closed: callers must report a pending
        cancellation instead of applying a destructive mutation concurrently.
        """
        if not session_id:
            return False
        ...
        entry = self._acquire_session_run_lock(session_id, timeout=timeout)
        if entry is None:
            return False
    ...
    def get_agent(
        ...
        """Get an agent while preserving the authenticated identity binding."""
        if session_id is None:
            if trusted_identity is not None:
                raise PermissionError("default agent cannot accept a session identity")
            with self._agents_lock:
                if self.default_agent is None:
                    self._init_default_agent()
                return self.default_agent

        self._assert_persisted_session_binding(
            session_id, conversation_owner_id
        )
        with self._agents_lock:
            agent = self.agents.get(session_id)
            if agent is None:
                self._init_agent_for_session(
                    session_id,
                    trusted_identity=trusted_identity,
                    conversation_owner_id=conversation_owner_id,
                )
                agent = self.agents[session_id]
            self._assert_agent_identity(
                agent, trusted_identity, conversation_owner_id
            )
            return agent

    @staticmethod
    def _assert_persisted_session_binding(
        session_id: str, conversation_owner_id: Optional[str]
    ) -> None:
        try:
            from agent.memory import get_conversation_store
            stored_owner = get_conversation_store().active_session_owner(session_id)
        except Exception as exc:
            # Security lookup failures must not degrade to session-id-only access.
            raise PermissionError("session ownership lookup failed") from exc
        if stored_owner is None:
            if conversation_owner_id is not None:
                raise PermissionError("owned session claim is missing")
            return
        if stored_owner != (conversation_owner_id or ""):
            raise PermissionError("session conversation owner mismatch")

    @staticmethod
    def _assert_agent_identity(
        agent: Agent, trusted_identity, conversation_owner_id: Optional[str] = None
    ) -> None:
        existing = getattr(agent, "identity_context", None)
        existing_owner = getattr(agent, "conversation_owner_id", None)
        if existing_owner != conversation_owner_id:
            raise PermissionError("session conversation owner mismatch")
        if trusted_identity is None:
            # Authenticated Web agents must never be recovered through an
            # untrusted session-id-only call path. Local/cloud legacy agents
            # retain their prior compatibility behavior.
            if existing is not None and str(existing.auth_source).startswith("web-"):
                raise PermissionError("trusted identity required for this session")
            return
        if existing is None or (
            existing.tenant_id != trusted_identity.tenant_id
            or existing.actor_user_id != trusted_identity.actor_user_id
        ):
            raise PermissionError("session identity mismatch")

    def _init_default_agent(self):
        """Initialize default super agent. Caller holds ``_agents_lock``."""
    ...
    def _init_agent_for_session(
        ...
        """Initialize one identity-bound agent. Caller holds ``_agents_lock``."""
    ...
    def sync_session_messages_from_store(
        ...
        try:
            ...
            remaining = store.load_messages(
                session_id, max_turns=10**6, owner_id=owner_id
            )
        except Exception as e:
            logger.warning(
    ...
    def agent_reply(self, query: str, context: Context = None, 
        ...
        try:
            ...
            registry = get_cancel_registry()
            # Every queued turn needs its own registry entry. IM channels may
            # not provide request_id, so using session_id as the key would let
            # the first completed turn unregister the queued turn's cancel
            # event. Session-wide cancellation still works through the
            # registry's session index.
            token_key = request_id or (
                f"{session_id}:run:{uuid.uuid4().hex}" if session_id else None
            )
            cancel_owner_id = (
            ...
            # constructs this object at the authentication boundary; raw client
            # identity claims are never copied into context.
            trusted_identity = (
                context.get("trusted_identity") if context else None
            )
            ...
            if web_owner_id is not None and not pre_persisted:
                raise RuntimeError(
            ...
            if session_id:
                ...
                if new_messages:
                    ...
                    if owner_id is not None and not persisted:
                        self.clear_session(session_id)
                        raise RuntimeError(
                            "authenticated Web response was not durably persisted"
                        )
    ...
    def _pre_persist_user_message(
        ...
        try:
            ...
            owner_id = (context.get("session_owner_id") or None) if context else None
            if clear_history:
                store.clear_session(session_id, owner_id=owner_id)
                if owner_id is not None:
    ...
    def remember_scheduled_output(
        ...
        if not self._persist_messages(
            session_id, messages, channel_type, owner_id=owner_id
        ):
            raise RuntimeError("scheduled output was not durably persisted")
        ...
        try:
            ...
            deleted = get_conversation_store().prune_scheduled_messages(
                session_id, keep_last_n=keep_last_n, owner_id=owner_id
            )
            if deleted:
                logger.debug(
        ...
        # Never mutate a cached Agent by session_id alone. Reload only from the
        # owner-checked durable store; if owner verification fails, memory stays
        # untouched and the caller receives a failure.
        if session_id in self.agents:
            synced = self.sync_session_messages_from_store(
                session_id, owner_id=owner_id
            )
            if synced < 0:
                raise RuntimeError("scheduled output cache sync failed")
    ...
    def has_cached_session(self, session_id: str) -> bool:
        with self._agents_lock:
            return bool(session_id and session_id in self.agents)
