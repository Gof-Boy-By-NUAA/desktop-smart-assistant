...
_LOCAL_ACTOR_USER_ID = "local-user"
_LOCAL_AUTH_SOURCE = "smart-assistant-local-single-user"
...
class AgentInitializer:
    ...
    def initialize_agent(
        ...
        Args:
            ...
            trusted_identity: 由可信认证边界构造的 IdentityContext；本机模式留空
        ...
        skill_identity = getattr(memory_manager, "identity_context", None)
        skill_manager = self._initialize_skill_manager(
            workspace_root,
            session_id,
            tenant_id=getattr(skill_identity, "tenant_id", None),
            identity_context=skill_identity,
        )
        ...
        prompt_lock_factory = getattr(
            skill_manager, "production_injection_lock", None
        )
        prompt_lock = (
            prompt_lock_factory()
            if prompt_lock_factory is not None
            else nullcontext()
        )
        with prompt_lock:
        ...
        if memory_manager:
            ...
            agent.identity_context = getattr(
                memory_manager, "identity_context", None
            )
            if hasattr(agent, 'model') and agent.model:
                memory_manager.flush_manager.llm_model = agent.model
        agent.session_id = session_id
        agent.conversation_owner_id = conversation_owner_id

        # Restore persisted conversation history for this session
        if session_id:
            self._restore_conversation_history(
                agent, session_id, owner_id=conversation_owner_id
            )
    ...
    def _restore_conversation_history(
        self, agent, session_id: str, owner_id: Optional[str] = None
    ) -> None:
        """
        ...
        try:
            ...
            saved = store.load_messages(
                session_id, max_turns=restore_turns, owner_id=owner_id
            )
            if saved:
    ...
    def _setup_memory_system(
        ...
        try:
            ...
            from agent.knowledge import GovernedKnowledgeRuntime
            ...
            if trusted_identity is not None and not isinstance(
                trusted_identity, IdentityContext
            ):
                raise TypeError("trusted_identity 必须是 IdentityContext")
            tenant_id = (
                trusted_identity.tenant_id
                if trusted_identity is not None
                else MemoryConfig().tenant_id
            )
            ...
            if trusted_identity is not None:
                ...
                # 单机模式没有独立登录用户；会话标识只用于会话范围隔离。
                # 本机身份只获得治理技能读取权，不获得提案、评测或发布权。
                roles = {"skill:read"}
                if session_id is None:
                    roles.update(
                        {"memory:write_shared", "knowledge:write_shared"}
                    )
                identity = IdentityContext(
                    tenant_id=memory_config.tenant_id,
                    actor_user_id=_LOCAL_ACTOR_USER_ID,
                    roles=frozenset(roles),
                    trace_id="agent-user-%s-%s" % (
                        _LOCAL_ACTOR_USER_ID,
                        uuid.uuid4().hex,
                    ),
            ...
            memory_manager.identity_context = identity

            knowledge_runtime = GovernedKnowledgeRuntime(
                workspace_root,
                tenant_id=memory_config.tenant_id,
            )
            memory_manager.knowledge_runtime = knowledge_runtime
            ...
            memory_tools = [
                ...
                MemoryGetTool(memory_manager, identity=identity, session_id=session_id),
                MemoryWriteTool(memory_manager, identity, session_id=session_id),
                MemoryRevokeTool(memory_manager, identity),
                MemoryRollbackTool(memory_manager, identity),
                KnowledgeSearchTool(
                    knowledge_runtime,
                    identity,
                    session_id=session_id,
                ...
                KnowledgeRevokeTool(knowledge_runtime, identity),
                KnowledgeRollbackTool(knowledge_runtime, identity),
            ]
        ...
        for tool_name in tool_manager.tool_classes.keys():
            try:
                ...
                if tool:
                    ...
                    if tool_name in ['read', 'write', 'edit', 'bash', 'search_files', 'ls', 'web_fetch', 'send', 'browser', 'vision']:
