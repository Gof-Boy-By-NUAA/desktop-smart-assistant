...
def start(foreground, no_logs):
    ...
    pid = _read_pid()
    if pid:
        click.echo(f"SmartAssistant is already running (PID: {pid}).")
        return
    ...
    if foreground:
        click.echo("Starting SmartAssistant in foreground...")
        if _IS_WIN:
        ...
        click.echo("Starting SmartAssistant...")
        ...
        _write_pid(proc.pid)
        click.echo(click.style(f"✓ SmartAssistant started (PID: {proc.pid})", fg="green"))
...
def stop():
    ...
    pid = _read_pid()
    if not pid:
        click.echo("SmartAssistant is not running.")
        return

    click.echo(f"Stopping SmartAssistant (PID: {pid})...")
    try:
        _kill_pid(pid)
        for _ in range(30):
            time.sleep(0.1)
    ...
    _remove_pid()
    click.echo(click.style("✓ SmartAssistant stopped.", fg="green"))
...
@click.pass_context
def update(ctx):
    """Update SmartAssistant and restart."""
    root = get_project_root()
    ...
    if _IS_WIN:
        ...
        subprocess.Popen(
            ["cmd.exe", "/c", "start", "SmartAssistant Update", "/wait", bat],
            cwd=root,
        )
        click.echo(click.style(
...
@click.command()
def status():
    """Show SmartAssistant running status."""
    ...
    pid = _read_pid()
    if pid:
        click.echo(click.style(f"● SmartAssistant is running (PID: {pid})", fg="green"))
    else:
        click.echo(click.style("● SmartAssistant is not running", fg="red"))
