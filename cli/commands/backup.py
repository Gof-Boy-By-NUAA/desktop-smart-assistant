"""Portable local backup and restore commands for SmartAssistant user data."""
...
BACKUP_FORMAT = "smart-assistant-backup"
BACKUP_VERSION = 1
...
def create_backup_archive(
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="smart-assistant-backup-"))
...
def _validate_archive(archive: zipfile.ZipFile) -> dict:
    ...
    if manifest.get("format") != BACKUP_FORMAT or manifest.get("version") != BACKUP_VERSION:
        raise ValueError("unsupported SmartAssistant backup format or version")
...
def restore_backup_archive(
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="smart-assistant-restore-"))
    try:
...
def restore_command(archive: Path, workspace: Optional[Path], yes: bool):
    ...
    if pid:
        raise click.ClickException(
            f"SmartAssistant is running (PID: {pid}). Run 'cow stop' before restoring."
        )
    if not yes:
        click.confirm(
